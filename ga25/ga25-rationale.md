...
While the governance action describes what Blockfrost is and what the services are that will be used for the Icebreakers network, it wasn’t clear exactly what the funds in this treasury withdrawal were being requested to do.  It does state under Deliverables, that “To ensure transparency and to prevent vendor lock-in, we are open-sourcing all components of the deliverables. This includes the infrastructure, libraries and documentation.” and so it was assumed that the requested amount is intended as a means to “purchase” the existing closed source software, so that it can be made open source and therefore available to anyone in the community.

While in this case we still found it to be “sufficient rationale”, we recommend that future treasury withdrawals are more explicit when describing their intended purpose regarding the use of treasury funds.
